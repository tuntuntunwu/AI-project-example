...
#自行选择以下参数改改 ，能跑就行
tf.app.flags.DEFINE_integer('embedding_dim', 300, 'dimension of word embedding')#词向量维度，不用更改
tf.app.flags.DEFINE_integer('batch_size', 25, 'number of example per batch')
tf.app.flags.DEFINE_integer('n_hidden', 400, 'number of hidden unit')
tf.app.flags.DEFINE_float('learning_rate', 0.001, 'learning rate')
tf.app.flags.DEFINE_integer('n_class', 3, 'number of distinct class')#分类数，不用改
tf.app.flags.DEFINE_integer('max_sentence_len', 80, 'max number of tokens per sentence')
tf.app.flags.DEFINE_float('l2_reg', 0.001, 'l2 regularization')
tf.app.flags.DEFINE_integer('display_step', 4, 'number of test display step')
tf.app.flags.DEFINE_integer('n_iter', 40, 'number of train iter')
tf.app.flags.DEFINE_float('keep_prob1', 0.8, 'dropout keep prob')
